...
from src.utils.evaluation_functions import evaluate_metrics, \
    add_interpretation_and_infos_to_explanation_file, load_json_param
...
def main():
    ...
    config["results_path"] = experiment_dir + "/" + config["explanation_file_base_name"]
    ...
    for ref in tqdm(reference_list):
        ...
        additional_infos = [np.ravel(prediction)]
        adv_image = xa.numpy()[0, 0]
        st_image = xst.numpy()[0, 0]
        add_interpretation_and_infos_to_explanation_file(
            explanation_file, ref, heatmap=heatmap,
            adv_image=adv_image, st_image=st_image, additional_infos=additional_infos)
    h5_db.close()
    explanation_file.close()

    run_evaluation = True
    if "run_evaluation" in list(config.keys()):
        run_evaluation = config["run_evaluation"]

    if run_evaluation:
        evaluate_metrics(
            config, explanation_path,
            metrics_name="all_iou", indexes_name=args.indexes_name, gt_type=gt_type)
        evaluate_metrics(
            config, explanation_path,
            metrics_name="ncc", indexes_name=args.indexes_name, gt_type=gt_type, unique=True)
