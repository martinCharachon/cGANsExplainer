...
class OptimizationCyCE(OptimizationInterpreterBase):
    """
    Model operator
    """
    ...
    def _set_tools(self):
        ...
        if "optimization-type" in list(self.training_config.keys()):
            if self.training_config["optimization-type"] not in ["together", "separated"]:
                raise AttributeError
            self._optimization_type = self.training_config["optimization-type"]
        else:
            self._optimization_type = "together"

    def _optimize_generators(self, inputs):
        if self._optimization_type == "together":
            self._optimize_generators_together(inputs)
        elif self._optimization_type == "separated":
    ...
    def _optimize_generators_together(self, inputs):
        with tf.GradientTape() as t0, tf.GradientTape() as t1:
            ...
            classif_adv_01_score = self.reference_model(x_adv_01)
            classif_adv_10_score = self.reference_model(x_adv_10)
            classif_cy_0110_score = self.reference_model(x_cy_0110)
            classif_cy_1001_score = self.reference_model(x_cy_1001)
            ...
            c_adv_01_loss = self._classification_loss(y_0_batch, classif_adv_01_score)
            c_adv_10_loss = self._classification_loss(y_1_batch, classif_adv_10_score)
            c_cy_0110_loss = self._classification_cyc_loss(y_0_batch, classif_cy_0110_score)
            c_cy_1001_loss = self._classification_cyc_loss(y_1_batch, classif_cy_1001_score)
            ...
            tot_loss = r_cy_0110_loss + r_cy_1001_loss + c_adv_01_loss + c_adv_10_loss + \
                       c_cy_0110_loss + c_cy_1001_loss + disc_0_loss + disc_1_loss
            gradient_0 = t0.gradient(tot_loss, self.generator_0.trainable_variables)
            gradient_1 = t1.gradient(tot_loss, self.generator_1.trainable_variables)

        self._optimizer.apply_gradients(zip(gradient_0, self.generator_0.trainable_variables))
        self._optimizer.apply_gradients(zip(gradient_1, self.generator_1.trainable_variables))
        ...
        losses = np.array([tot_loss.numpy(),
                           ...
                           disc_0_loss.numpy(), disc_1_loss.numpy()])
    ...
    def _optimize_generator_0(self, inputs):
        with tf.GradientTape() as t:
            ...
            disc_1_loss = self._adversarial_generator_loss(discriminator_01_score)
            tot_loss = c_adv_01_loss + disc_1_loss + \
        ...
        losses = np.array([tot_loss.numpy(), c_adv_01_loss.numpy(), disc_1_loss.numpy(),
    ...
    def _optimize_generator_1(self, inputs):
        with tf.GradientTape() as t:
            ...
            disc_0_loss = self._adversarial_generator_loss(discriminator_10_score)
            tot_loss = c_adv_10_loss + disc_0_loss + \
        ...
        losses = np.array([tot_loss.numpy(), c_adv_10_loss.numpy(),
                           disc_0_loss.numpy(), r_cy_1001_loss.numpy(), c_cy_1001_loss.numpy()])
    ...
    def _validate(self, inputs):
        ...
        tot_loss_0 = c_adv_01_loss + disc_gen_1_loss + \
                     r_cy_0110_loss + c_cy_0110_loss
        tot_loss_1 = c_adv_10_loss + disc_gen_0_loss + \
                     r_cy_1001_loss + c_cy_1001_loss
