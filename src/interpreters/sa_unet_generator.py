...
class StAdvUNetGenerator(BaseModel):
    def __init__(self,
                 input_shape,
                 ...
                 final_convolution_block_filters=[32, 16],
        ...
        self._decoder_block = UNetDecoderBlock(
            ...
            dropout_type=dropout_type)
        self._output_block_st = OutputblockGenerator(
        ...
        self._output_block_adv = OutputblockGenerator(
            input_shape=self._input_shape, final_convolution_layer=final_convolution_layer,
            final_convolution_block_filters=final_convolution_block_filters,
            data_format=data_format)
        ...
        super(StAdvUNetGenerator, self).__init__(*args, **kwargs)
    ...
    def model(self, *args, **kwargs):
        ...
        return Model(inputs=image_inputs, outputs=decoded_outputs, name="interpreter")
    ...
    def _build_decoder_model(self):
        inputs = [Input(shape=ref_layer_shape) for ref_layer_shape in
                  self.decoder_inputs_shape_list]
        decoded_output = self._decoder_block(inputs)
        out_st = self._output_block_st(decoded_output)
        out_adv = self._output_block_adv(decoded_output)
        return Model(inputs=inputs, outputs=[out_st, out_adv], name="decoder")
